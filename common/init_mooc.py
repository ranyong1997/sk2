...
class Login:
    ...
    def to_url(self, name, password):
        data = {
            "userName": name,
            "password": password,
            "type": 1
        }
        scrf_res = self.request_session.get(
            url="https://icve-mooc.icve.com.cn/learning/u/student/student/mooc_index.action", headers=self.header)
        print(scrf_res.json())

    def login(self, name, password):  # 0.登录
        """
        登录
        :param name: 用户名
        :param password: 密码
        ...
        while login_fail_num < 6:
            result = self.to_url(name, password)
...
if __name__ == '__main__':
    ...
    for key, value in login.get_user_all().items():
        ...
        login.to_url(name=username, password=password)

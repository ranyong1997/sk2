...
def join_course(session, token, course_id):
    url = "https://mooc.icve.com.cn/patch/zhzj/portalMooc_addCourseFormStudent.action"
    data = {
        'courseId': course_id,
        'token': token,
        'status': 'running'
...
def openLearnResItem(id, type, w=None, c=None):
    try:
        ...
        if type == "video" or type == 'courseware':
            ...
            logger.info(
                "\t\t\t\t ~~~~>执行结果: %s --- %s", learn_detail_record['data']['timeRecordResult']['msg'],
...
def run(username, password, topic_content, jump_content, type_value, course_id):
    separator = "*" * 40
    ...
    logger.info(f"\t>>> 课程获取中...")
    # 甭管有没有加入，先执行加入课程
    join_course(session, token, course_id)
    logger.info(f"\t>>>账号:{username}加入课程{course_id}成功...")
    time.sleep(1)
    # 获取课程列表
